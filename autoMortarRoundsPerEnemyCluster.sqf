...
private _detection_distance = if (count _this > 1) then {_this select 1} else {800};
private _rounds = if (count _this > 2) then {_this select 2} else {8};
private _cluster_radius = if (count _this > 3) then {_this select 3} else {50};
private _number_enemy_clusters = if (count _this > 4) then {_this select 4} else {8};
private _cool_down_for_effect = if (count _this > 5) then {_this select 5} else {60};
private _unlimited_ammo = if (count _this > 6) then {_this select 6} else {false};
...
private _accuracy_radius = if (count _this > 7) then {_this select 7} else {0};
...
fnc_getAmmoCount = {
    ...
    private _count = 0;
    {if (_x select 0 == _ammoType) exitWith {_count = _x select 1}} forEach magazinesAmmo _vehicle;
    _count
};
...
fnc_getEnemies = {
    ...
    _enemies select {alive _x && {side _x == east}}
};

fnc_getCluster = {
    params ["_unit", "_radius"];
    private _cluster = (getPos _unit) nearEntities ["Man", _radius];
    _cluster select {alive _x && {side _x == east}}
};
...
fnc_fireMortar = {
    ...
    if (!canFire _mortar) exitWith {false};
...
[
    ...
    while {alive _mortar} do {
        ...
        if (count _enemies == 0) then {continue};
